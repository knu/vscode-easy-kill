...
suite("Character Search Tests", () => {
  ...
  suite("f/F (forward/backward inclusive) navigation", () => {
    test("f SPC, f, F, F, F, f, t, t, F sequence", async () => {
      ...
      await withEditor("aa bb cc dd ee ff", async (editor) => {
        ...
        assert.strictEqual(editor.document.getText(sel.range), "dd ", "f SPC should select 'dd '");
        currentSel = sel;

        // Step 2: f -> "dd ee "
        sel = await forwardInclusiveBounds.getNewSelection(editor, currentSel, 1);
        assert.ok(sel, "f should expand to next space");
        assert.strictEqual(editor.document.getText(sel.range), "dd ee ", "f should select 'dd ee '");
        currentSel = sel;
        ...
        assert.strictEqual(editor.document.getText(sel.range), "dd ", "F should shrink back to 'dd '");
        currentSel = sel;
        ...
        assert.strictEqual(editor.document.getText(sel.range), " ", "F should select space before dd");
        currentSel = sel;
        ...
        assert.strictEqual(editor.document.getText(sel.range), " cc ", "F should select ' cc '");
        currentSel = sel;
        ...
        assert.ok(sel, "f type change should succeed");
        assert.strictEqual(editor.document.getText(sel.range), " ", "f should shrink to space before dd");
        ...
        assert.strictEqual(editor.document.getText(sel.range), "dd", "t should select 'dd' (exclusive of space)");
        ...
        assert.strictEqual(editor.document.getText(sel.range), "dd ee", "t should select 'dd ee' (exclusive)");
        currentSel = sel;

        // Step 9: F (type change to backward inclusive) -> "dd "
        sel = await backwardInclusiveBounds.getNewSelection(editor, currentSel, undefined);
        assert.ok(sel, "F type change should succeed");
        assert.strictEqual(editor.document.getText(sel.range), "dd ", "F should select 'dd ' (inclusive of space)");
        currentSel = sel;
      });
    });
  });
  ...
  suite("Basic character search", () => {
    test("forward inclusive finds character", async () => {
      await withEditor("hello world", async (editor) => {
        ...
        assert.ok(sel);
        assert.strictEqual(editor.document.getText(sel.range), "hello");
      });
    });
    ...
    test("forward exclusive finds character", async () => {
      await withEditor("hello world", async (editor) => {
        ...
        assert.ok(sel);
        assert.strictEqual(editor.document.getText(sel.range), "hell");
      });
    });
  });
});
